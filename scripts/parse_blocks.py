...
SECTION_HEADERS = [
    ...
    ("J", re.compile(r"J\.?\s+(?:DEGREES?|DISCIPLINARY)\s+(?:CONFERRED|AREAS?)", re.I)),
]
...
def main():
    ...
    txt_files = sorted(glob.glob(os.path.join(TXT_DIR, 'cds-*.txt')), reverse=True)
    summary = {}
    for path in txt_files:
        year = os.path.basename(path)[len('cds-'):-len('.txt')]
        result = parse_year(year)
        if not result:
            print(f'  [skip] {year}: parse_year returned no result')
            continue
